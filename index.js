...
if (!hasAccount) {
  ...
  db.prepare(`UPDATE mensajes SET account = 'wa1' WHERE account IS NULL;`).run();
}
db.prepare(`CREATE INDEX IF NOT EXISTS idx_mensajes_account ON mensajes(account)`).run();
...
function getChromiumPath() {
  ...
  console.warn('⚠️ No se encontró chromium; Puppeteer usará su valor por defecto');
  return undefined;
}
...
function initClient(id) {
  ...
  c.on('qr', async (qr) => {
    try { clients[id].qr = await qrcode.toDataURL(qr); clients[id].ready = false; console.log(`📲 QR listo para ${id}`); }
    catch (e) { console.error(`[${id}] QR error:`, e.message); }
  });
  c.on('authenticated', () => { clients[id].authed = true; clients[id].qr = null; console.log(`🔐 [${id}] authenticated`); });
  c.on('ready', () => { clients[id].ready = true; clients[id].qr = null; clients[id].state='CONNECTED'; console.log(`✅ ${id} ready`); });
  c.on('change_state', (state) => { clients[id].state = state; clients[id].ready = state==='CONNECTED'; if (clients[id].ready) clients[id].qr=null; console.log(`🔄 [${id}] state: ${state}`); });
  c.on('auth_failure', (m) => { clients[id].ready=false; clients[id].authed=false; clients[id].qr=null; console.error(`❌ [${id}] auth_failure:`, m); });
  c.on('disconnected', (reason) => {
    clients[id].ready=false; clients[id].state='DISCONNECTED';
    console.warn(`⚠️ [${id}] desconectado: ${reason}`);
    setTimeout(() => { try { c.initialize(); } catch(e){ console.error(`[${id}] reinit:`, e.message); } }, 5000);
  });

  setInterval(async () => {
    try { const st = await c.getState(); clients[id].state = st || clients[id].state; clients[id].ready = st==='CONNECTED'; if (clients[id].ready) clients[id].qr=null; } catch {}
  }, 5000);

  c.initialize().catch(e => {
    console.error(`💥 [${id}] init falló:`, e.message);
    setTimeout(() => { try { c.initialize(); } catch (e2) { console.error(`[${id}] 2º init falló:`, e2.message); } }, 5000);
  });
}
...
function st(acc, res) { if (!clients[acc]) { res.status(404).json({ error: 'Cuenta desconocida' }); return null; } return clients[acc]; }
...
app.get('/:acc/packages', async (req, res) => {
  ...
  if (!ACCOUNTS[acc]) return res.status(404).json({ ok:false, error:'account_not_found' });
  try {
    const url = `${API_BASE}${ACCOUNTS[acc].packagesPath}`;
    const r   = await fetchPackages(url);
    const raw = await r.text();
    if (!r.ok) return res.status(r.status).json({ ok:false, status:r.status, message: raw.slice(0,200) });
    try { JSON.parse(raw); } catch { return res.status(502).json({ ok:false, message:'Respuesta no JSON (API)' }); }
...
app.post('/:acc/send', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success:false, error:'send_failed', detail:e.message });
  }
});
...
app.post('/:acc/mensajes', (req, res) => {
  ...
  if (!ACCOUNTS[acc]) return res.status(404).json({ success:false, error:'account_not_found', acc });
  if (!texto)        return res.status(400).json({ success:false, error:'empty' });
  db.prepare('INSERT INTO mensajes(texto, account) VALUES(?, ?)').run(texto, acc);
  res.json({ success:true });
});
...
app.get('/:acc/mensajes', (req, res) => {
  ...
  if (!ACCOUNTS[acc]) return res.status(404).json({ success:false, error:'account_not_found', acc });
  const rows = db.prepare('SELECT id, texto FROM mensajes WHERE account = ? ORDER BY id DESC').all(acc);
  res.json(rows);
});
...
app.post('/:acc/enviar-excel', upload.single('excel'), async (req, res) => {
  const acc = (req.params.acc || '').toLowerCase();
  if (!ACCOUNTS[acc]) return res.json({ success:false, message:'account_not_found' });
  const s = clients[acc];
  ...
  try {
    ...
    const base = db.prepare('SELECT texto FROM mensajes WHERE account = ? ORDER BY id DESC').all(acc)
    ...
    const go = async () => {
      ...
      } catch (e) { console.error('Excel send error:', acc, e.message); }
      i++;
...
app.use((req, res) => {
  res.status(404).json({ ok:false, error:'route_not_found', method:req.method, url:req.originalUrl });
});
